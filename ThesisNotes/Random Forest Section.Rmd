...
We are using a data set from a survey taken in the MAT 111 class (Elementary Probability and Statistics). This data set has 71 rows and 12 variables. The survey includes variables such as sex, height, GPA, sleep, and the fastest speed ever driven. The names of some of the variables may seem a little odd, such as weight_feel, love_first, and  extra_life. The weight_feel variable is how the participant feels about their weight. They could have answered underweight (which is given a numerical value of 1), about right (which is given a numerical value of 2), or overweight (which is given a numerical value of 3). The love_feel variable is whether or not the participant believes in love at first sight and the extra_life variable is whether or not the participant believes in extraterrestrial life. 
...
rf.sexm111 <- randomForest(sex~fastest+GPA+height+sleep+weight_feel+love_first, data=m111surv2, do.trace = 50)
rf.sexm111
