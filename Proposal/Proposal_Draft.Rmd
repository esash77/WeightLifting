...
![Figure 1: A Unilateral Dumbbell Bicep Curl](http://www.dumbbell-exercises.com/exercises/biceps/images/2.gif)
  ...
  Dr. White designed a model that predicted the activity the subject was performing based on the data gathered during each of the lift types. The final model he constructed was estimated to be correct about 99.7% of the time. In the parameters of the project, Dr. White built a model using observations from the same lifts as the observations for which the activity type was predicted. However, this is not particularly useful in a real-life situation. A real-life model for predicting activity type would always be making predictions for observations for new lifts.
