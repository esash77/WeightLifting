...
not_summary_variable <- sapply(wl, FUN = goodVar) # This keeps all the variables 
                  # that do not have missing values

goodNames <- names(not_summary_variable[not_summary_variable])

keepNames <- goodNames[-c(1,3,4,5,6)] # This keeps all the useful variables in 
                  # the data set and removes the extra variables that are 
                  # not useful

wl2 <- wl[,keepNames] # We will put all the variables we wish to keep into a new
                  # data set so as to preserve the original data set

wl2$user_name <- factor(wl$user_name) # Makes user_name and classe a factor variable 
wl2$classe <- factor(wl$classe)     # because the randomForest function requires factors

wn <- wl$num_window # I will want to separate my data into training and test sets based
...
partition_var <- function(data, var){
  vals <- unique(var) # Creates an object of only the variable
  
  n <- length(vals)
  m <- floor(2/3*n)
  
  bools <- c(rep(TRUE,m), rep(FALSE,n-m))  # A booliean object that has m TRUE
                      # values and n-m FALSE values
  
  inTrain <- sample(bools, size = n, replace = FALSE) # A random sample of the bools
                      # with size n and no replacement
  
  trvals <- vals[inTrain] # Gives values to the list of TRUEs from the inTrain 
                      # random sample. 
  
  vartr <- var %in% trvals # A vector of TRUEs and FALSEs
  
  tr <- data[vartr,] # Training data set
  
  tst <- data[!vartr,] # Test data set
...
```{r cache = T}
set.seed(2020)
results <- partition_var(wl2, wn) # I put the num_window variable into a new object, wn 

wlTrain <- results[[1]] # Since 2 results are given (test and training set), 
                        # the first result is the training set

wlTest <- results[[2]]  # The second result is the test set
...
**XXXXX What exactly does this mean? Explain**
